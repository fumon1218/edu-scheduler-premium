...
export default function App() {
  ...
  const filteredSchedules = useMemo(() => {
    return schedules.filter(s => {
      const matchesSearch = 
        [s.program, s.location, s.target].some(v => (v || '').toLowerCase().includes(searchTerm.toLowerCase()));
  ...
  const resetForm = () => {
    setFormData({
      ...
      program: PROGRAMS[0], location: LOCATIONS[0], target: TARGETS[0]
    });
    setEditingId(null);
    setIsEditing(false);
  };
    ...
    <div className="flex h-screen bg-bg-primary overflow-hidden font-sans">
      ...
      <aside className="hidden lg:flex w-64 bg-sidebar-bg border-r border-border-color flex-col p-6 shrink-0">
        ...
        <nav className="flex-1 space-y-1">
          ...
          <div className="px-4 py-2.5 text-text-muted hover:bg-gray-50 rounded-lg text-sm font-medium cursor-pointer transition-colors flex items-center gap-3"><Settings size={18} /><span>설정</span></div>
        </nav>
          ...
          <div className="max-w-7xl mx-auto">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8">
              <div className="flex items-center gap-6">
                <div>
                  <h2 className="text-2xl font-bold text-text-main">{viewMode === 'list' ? '스케줄 관리' : `${safeFormat(baseDate, 'yyyy년 M월')} 일정표`}</h2>
